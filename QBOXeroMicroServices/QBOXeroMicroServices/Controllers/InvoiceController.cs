...
namespace QBOXeroMicroServices.Controllers
{
    ...
    public class InvoiceController : ControllerBase
    {
        ...
        {
            var objResponse = new XeroInvoiceSyncResult();
            try
            {
                if (model == null)
                {
                ...
                using (var httpClient = new HttpClient())
                {
                    var json = JsonConvert.SerializeObject(model);
                    var content = new StringContent(json, Encoding.UTF8, "application/json");
                    var response = await httpClient.PostAsync(AppConfiguration.XeroInvoiceGet, content);

                    if (response.IsSuccessStatusCode)
                    {
                        var result = await response.Content.ReadAsStringAsync();
                        objResponse = JsonConvert.DeserializeObject<XeroInvoiceSyncResult>(result);
                    }
                    else
                    {
                        return StatusCode((int)response.StatusCode);
        ...
        [HttpPost]
        [Route("/InsertUpdateXeroInvoice")]
        public async Task<IActionResult> InsertUpdateXeroInvoice([FromBody] XeroInvoiceInsertUpdateReqViewModel model)
        {
            var objResponse = new XeroInvoiceSyncResult();
            try
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    var json = JsonConvert.SerializeObject(model);
                    var content = new StringContent(json, Encoding.UTF8, "application/json");
                    var response = await httpClient.PostAsync(AppConfiguration.XeroInvoiceInsertUpdate, content);
        ...
        [HttpDelete]
        [Route("/DeleteXeroInvoice")]
        public async Task<IActionResult> DeleteXeroInvoice([FromBody] XeroInvoiceDeleteReqViewModel model)
        {
            var objResponse = new DownloadResultPT();
            try
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    var json = JsonConvert.SerializeObject(model);
                    var content = new StringContent(json, Encoding.UTF8, "application/json");
                    var response = await httpClient.PostAsync(AppConfiguration.XeroInvoiceDelete, content);
